...
.PARAMETER DbPath
    Path to an exported SQLite database file, or a folder containing multiple .db files.
    When a folder is specified, all .db files are analyzed together and results merged.
    Defaults to './dbs' in the current directory.
...
# ============================================================
# Validation: DB path exists (file or folder)
# ============================================================
$dbPathIsFolder = $false
if (Test-Path $DbPath -PathType Container) {
    $dbPathIsFolder = $true
    $dbFiles = @(Get-ChildItem $DbPath -Filter *.db | Sort-Object LastWriteTime | ForEach-Object { $_.FullName })
    if ($dbFiles.Count -eq 0) {
        Write-Error "No .db files found in folder: $DbPath"
        exit 1
    }
    Write-Verbose "Found $($dbFiles.Count) database(s) in folder: $DbPath"
} elseif (Test-Path $DbPath -PathType Leaf) {
    $dbFiles = @((Resolve-Path $DbPath).ProviderPath)
} else {
    Write-Error "Database path not found: $DbPath"
    exit 1
}
$resolvedDbPath = $dbFiles[0]  # default for single-file operations
...
# ============================================================
# Helper: Build conversation title lookup
# ============================================================
function Get-ConversationTitles {
    param([string]$Db)

    $titles = @{}

    # Source 1: Direct conversation title attribute (if Copilot emits one)
    $sql = @"
SELECT DISTINCT sa.value AS title, s.conversation_id
FROM spans s
...
"@
    $rows = Invoke-SqliteQuery -Sql $sql -Db $Db 2>''
    if ($rows) {
        foreach ($r in $rows) {
            if ($r.conversation_id -and -not $titles.ContainsKey($r.conversation_id)) {
                $titles[$r.conversation_id] = $r.title
            }
        }
    }

    # Source 2: First user_request per conversation (fallback title)
    $sql2 = @"
SELECT DISTINCT s.conversation_id,
    (SELECT sa2.value FROM span_attributes sa2 WHERE sa2.span_id = s.span_id AND sa2.key = 'copilot_chat.user_request') AS first_request
FROM spans s
WHERE s.conversation_id IS NOT NULL
  AND s.name LIKE 'invoke_agent%'
ORDER BY s.start_time_ms
"@
    $rows2 = Invoke-SqliteQuery -Sql $sql2 -Db $Db 2>''
    if ($rows2) {
        $seen = @{}
        foreach ($r in $rows2) {
            if ($r.conversation_id -and -not $titles.ContainsKey($r.conversation_id) -and -not $seen.ContainsKey($r.conversation_id)) {
                $seen[$r.conversation_id] = $true
                if ($r.first_request) {
                    $firstLine = ($r.first_request -split '\n')[0].Trim()
                    if ($firstLine.Length -gt 0) {
                        if ($firstLine.Length -gt 120) {
                            $titles[$r.conversation_id] = $firstLine.Substring(0, 120) + "..."
                        } else {
                            $titles[$r.conversation_id] = $firstLine
                        }
                    }
                }
            }
...
$report = [ordered]@{
    ...
    db_path      = if ($dbPathIsFolder) { (Resolve-Path $DbPath).ProviderPath } else { $resolvedDbPath }
    data         = @()
}

# Build conversation title lookup across all databases
$convTitles = @{}
foreach ($db in $dbFiles) {
    $partial = Get-ConversationTitles -Db $db
    foreach ($key in $partial.Keys) {
        if (-not $convTitles.ContainsKey($key)) {
            $convTitles[$key] = $partial[$key]
        }
    }
}

if ($Sessions) {
    # Per-session breakdown from invoke_agent spans (multi-DB)
    $allSessionData = @()
    foreach ($db in $dbFiles) {
        $sql = @"
SELECT
...
"@
        if ($TopN -gt 0 -and $dbFiles.Count -eq 1) { $sql += " LIMIT $TopN" }
        $agentSpans = Invoke-SqliteQuery -Sql $sql -Db $db
        ...
        foreach ($span in $agentSpans) {
            ...
            $attrs = Invoke-SqliteQuery -Sql $attrSql -Db $db
            ...
            $allSessionData += [ordered]@{
                ...
                conversation_title = $convTitles[$span.conversation_id]
                session_summary = $summary
            }
        }
    }
    # Sort by timestamp and apply TopN after merge
    $allSessionData = @($allSessionData | Sort-Object timestamp -Descending)
    if ($TopN -gt 0) { $allSessionData = @($allSessionData | Select-Object -First $TopN) }
    $report.data = $allSessionData
}
elseif ($Cost) {
    # Cost breakdown by model — per-span processing across all DBs
    $modelAgg = @{}
    foreach ($db in $dbFiles) {
        $sql = @"
...
"@
        # When scanning multiple DBs, skip per-DB LIMIT (apply post-merge)
        if ($TopN -gt 0 -and $dbFiles.Count -eq 1) { $sql += " LIMIT $TopN" }
        $chatSpans = Invoke-SqliteQuery -Sql $sql -Db $db
    ...
    $sortedModels = @($modelAgg.Keys | Sort-Object { $modelAgg[$_].input } -Descending)
    $modelCount = 0
    foreach ($modelKey in $sortedModels) {
        ...
        $modelCount++
        if ($TopN -gt 0 -and $modelCount -gt $TopN) { continue }
...
elseif ($Prompts) {
    # Export full LLM prompt content per session (multi-DB)
    $allPromptData = @()
    foreach ($db in $dbFiles) {
        $sql = @"
SELECT
...
"@
    ...
    # Sort by timestamp and apply TopN after merge
    $allPromptData = @($allPromptData | Sort-Object timestamp -Descending)
    if ($TopN -gt 0) { $allPromptData = @($allPromptData | Select-Object -First $TopN) }
    $report.data = $allPromptData
    # Add summary stats
    ...
    # Daily / Weekly / Monthly aggregation — per-span processing across all DBs
    $groupLabel = "date"
    ...
    function Get-Period {
        ...
        elseif ($Monthly) { return $dt.ToString('yyyy-MM') }
        else { return $dt.ToString('yyyy-MM-dd') }
    }

    # Aggregate per-span costs by period, then by model across all DBs
    $periods = [ordered]@{}
    foreach ($db in $dbFiles) {
        $sql = @"
...
"@
    ...
    # Sort periods chronologically and apply TopN
    $sortedPeriods = @($periods.Values | Sort-Object { $_[$groupLabel] } -Descending)
    if ($TopN -gt 0) { $sortedPeriods = @($sortedPeriods | Select-Object -First $TopN) }
    $report.data = $sortedPeriods
}
